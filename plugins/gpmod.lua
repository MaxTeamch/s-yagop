...
local function generate_link(cb_extra, success, result)
  ...
  --lock/unlock group bots. bot automatically kicks bots when joining
  local function lock_group_bots(msg, data)
    if not is_momod(msg) then
      return "For moderators only!"
    end
    local group_bots_lock = data[tostring(msg.to.id)]['settings']['lock_bots']
    if group_bots_lock == 'yes' then
      return 'Group bots is already locked'
    else
  ...
  local function unlock_group_bots(msg, data)
    if not is_momod(msg) then
      return "For moderators only!"
    end
    local group_bots_lock = data[tostring(msg.to.id)]['settings']['lock_bots']
  ...
  --lock/unlock group link. link automatically delete.
  ...
  local function unlock_group_link(msg, data)
    ...
    if group_link_lock == 'no' then
      return 'Group link is not locked'
    else
      data[tostring(msg.to.id)]['settings']['lock_link'] = 'no'
      save_data(_config.moderation.data, data)
      return 'Group link unlocked'
    end
  end
  ...
  function run(msg, matches)
    ...
    if data[tostring(msg.to.id)] then
      ...
      if matches[1] == 'group' and matches[2] == '+' then --group lock *
        ...
        if matches[3] == 'bots' then
          return lock_group_bots(msg, data)
        end
      ...
      if matches[1] == 'group' and matches[2] == '-' then --group unlock *
        ...
        if matches[3] == 'bots' then
          return unlock_group_bots(msg, data)
        end
	if matches[3] == 'link' then
      ...
      if matches[1] == 'chat_rename' then
        ...
        if group_name_lock == 'yes' then
          if group_name_set ~= tostring(msg.to.print_name) then
            if not is_chan_msg(msg) then
              to_rename = 'chat#id'..msg.to.id
              rename_chat(to_rename, group_name_set, ok_cb, false)
            else
              to_rename = 'channel#id'..msg.to.id
      ...
      if matches[1] == 'chat_add_user' or matches[1] == 'chat_add_user_link' then
        ...
        local group_bots_lock = settings.lock_bots
        local userobj = msg.action.user or msg.from
        local user = 'user#id'..userobj.id
        local chat
        if not is_mod(userobj.id, msg.to.id) then
          if not is_chan_msg(msg) then
  ...
  return {
    ...
    usage = {
      ...
      "!group <+|-> bots : Lock/unlock adding bots to group",
