...
---------------------------------------------------------------------------
  local function setowner(receiver, member_username, member_id)
    local data = load_data(_config.moderation.data)
    local group = string.gsub(receiver, 'chat#id', '')
    group = string.gsub(group, 'channel#id', '')
    if not data[group] then
    ...
    if data[group]['owners'][tostring(member_id)] then
      return send_large_msg(receiver, member_username..' is already a owner.')
    end
    data[group]['owners'][tostring(member_id)] = member_username
    save_data(_config.moderation.data, data)
    return send_large_msg(receiver, '@'..member_username..' has been owner.')
  end

  local function deowner(receiver, member_username, member_id)
    local data = load_data(_config.moderation.data)
    local group = string.gsub(receiver, 'chat#id', '')
    group = string.gsub(group, 'channel#id', '')
    if not data[group] then
    ...
    if not data[group]['owners'][tostring(member_id)] then
      return send_large_msg(receiver, member_username..' is not a owner.')
    end
    data[group]['owners'][tostring(member_id)] = nil
    save_data(_config.moderation.data, data)
    return send_large_msg(receiver, '@'..member_username..' has been deownerd.')
  end

  local function setowner_reply(extra, success, result)
    if result.from.username then
      setowner(extra, result.from.username, result.from.peer_id)
    else
      setowner(extra, result.from.first_name, result.from.peer_id)
    end
  end

  local function deowner_reply(extra, success, result)
    if result.from.username then
      deowner(extra, result.from.username, result.from.peer_id)
    else
      deowner(extra, result.from.first_name, result.from.peer_id)
    end
  end
---------------------------------------------------------------------------------
  ...
  local function resolved_username(cb_extra, success, result)
    ...
    if success == 1 then
	  ...
	  elseif mod_cmd == 'setowner' then
        return setowner(receiver, member_username, member_id)
      elseif mod_cmd == 'deowner' then
        return deowner(receiver, member_username, member_id)
  ...
  local function ownerlist(msg)
    ...
    local message = 'List of owners for ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
  ...
  function run(msg, matches)
	...
	-----------------------------------------------------------------------------------
	------------------------------------------------------------------------------------
	if matches[1] == 'setowner' then -- and matches[2] then
      if not is_admin(msg) then
        return "Only admins can promote"
      end
      if msg.reply_id then
        get_message(msg.reply_id, setowner_reply, get_receiver(msg))
        return nil
      end
    ...
    if matches[1] == 'deowner' then -- and matches[2] then
      if not is_admin(msg) then
        return "Only admins can demote"
      end
      if msg.reply_id then
        get_message(msg.reply_id, deowner_reply, get_receiver(msg))
        return nil
      end
  ...
  return {
    ...
    usage = {
      ...
      admin = {
		...
		"!setowner <username> : setowner user as owner",
        "!deowner <username> : deowner user from owner",
        "#setowner (by reply) : setowner user as owner",
        "#deowner (by reply) : deowner user from owner",
        "!ownerlist : List of owners",
      },
      sudo = {
