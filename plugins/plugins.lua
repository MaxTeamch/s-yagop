do
  ...
  local function enable_plugin( plugin_name )
    ...
    if plugin_enabled(plugin_name) then
      return str2emoji(':information_source:')..' Plugin '..plugin_name..' is enabled'
    end
    ...
    if plugin_exists(plugin_name) then
      ...
      return str2emoji(':exclamation:')..' Plugin '..plugin_name..' does not exists'
    end
  end
  ...
  local function disable_plugin( name, chat )
    ...
    if not plugin_exists(name) then
      return str2emoji(':exclamation:')..' Plugin '..name..' does not exists'
    end
    local k = plugin_enabled(name)
    -- Check if plugin is enabled
    if not k then
      return str2emoji(':information_source:')..' Plugin '..name..' not enabled'
    end
    -- Disable and reload
  ...
  local function disable_plugin_on_chat(receiver, plugin)
    if not plugin_exists(plugin) then
      return str2emoji(':exclamation:').." Plugin doesn't exists"
    end
    ...
    save_config()
    return str2emoji(':information_source:')..' Plugin '..plugin..' disabled on this chat'
  end

  local function reenable_plugin_on_chat(receiver, plugin)
    if not _config.disabled_plugin_on_chat then
      return str2emoji(':information_source:')..' There aren\'t any disabled plugins'
    end

    if not _config.disabled_plugin_on_chat[receiver] then
      return str2emoji(':information_source:')..' There aren\'t any disabled plugins for this chat'
    end

    if not _config.disabled_plugin_on_chat[receiver][plugin] then
      return str2emoji(':exclamation:')..' This plugin is not disabled'
    end
  ...
  local function run(msg, matches)
    ...
    if matches[1] == 'disable' and is_sudo(msg) then --after changed to moderator mode, set only sudo
      if matches[2] == 'plugins' then
        return str2emoji(':no_entry_sign:')..' This plugin can\'t be disabled'
      end
